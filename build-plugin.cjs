...
class WordPressPluginBuilder {
    ...
    async build() {
        try {
            console.log('🔨 ЗБІРКА WORDPRESS ПЛАГІНА CATALOG MASTER v1.1.4\n');
...
async function main() {
    ...
    if (args.includes('--help') || args.includes('-h')) {
        console.log(`
🔨 WordPress Plugin Builder для Catalog Master v1.1.4
...
`);
    ...
    if (args.includes('--version') || args.includes('-v')) {
        console.log('WordPress Plugin Builder v1.1.4 (Vite Edition)');
        return;
    }
